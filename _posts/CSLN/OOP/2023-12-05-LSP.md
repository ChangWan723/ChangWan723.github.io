...
**Inheritance**, while one of the pillars of object-oriented programming, **is invasive**. This is because all non-private attributes and methods of the parent class are visible to the child class, which means that the child class can modify these at will.
...
- In fact, even without considering LSPs, **overriding non-abstract methods is still strange behaviour**. Because:
  - A non-abstract method in a parent class can be seen as a convention, which represents itself and its child classes should have such behaviour. **Overriding a non-abstract method is a violation of the convention.**
  ...
  - If a method of a class may be overridden, it means that the method is not a common feature. **Overriding a non-abstract method indicates that the method shouldn't be implemented in the parent class.**
...
> Subclasses should extend, rather than modify, their parent's functionality. (This is also compliance with the [OCP](/posts/OCP/))
{: .prompt-tip }
